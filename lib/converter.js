...
//
function convertMediumFile(filePath, outputFolder) {
...
module.exports = {
  convert: function(srcPath, outputFolder = '.') {
    ...
    if (isDir) {
      ...
      fs.readdirSync(srcPath).forEach(file => {
        if (file.endsWith('.html')) {
          convertMediumFile(path.resolve(file), outputFolder);
        }
      });
    } else {
      convertMediumFile(path.resolve(srcPath), outputFolder);
    }
  },
};
