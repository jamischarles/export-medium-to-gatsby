...
// primary entry point
async function convertMediumFile(filePath, outputFolder, templatePath) {
  ...
  try {
    var postData = await gatherPostData(content, options);
...
module.exports = {
  convert: async function(srcPath, outputFolder = '.', templatePathStr) {
    ...
    if (isDir) {
      ...
      fs.readdirSync(srcPath).forEach(file => {
        ...
        if (file.endsWith('.html')) {
          promises.push(convertMediumFile(curFile, outputFolder, templatePath));
          // } else {
